...
#' @export
aracne2regulon_debug <- function(afile, eset, gene=F, format=c("adj", "3col")[1], verbose=T) {
	require(mixtools)
	if (verbose) cat("\nLoading the dataset...")
  if (length(eset)==1) {
...
##################################################################################################
################################ F U N C I Ó N  M O D I F I C A D A  #############################
##################################################################################################
...
TFscore <- function (regul, mu = NULL, sigma = NULL, verbose=T) {
  ...
  ##################################################################################################
  #---------------------------------------------------------------------------------------------------
  # Se cambió 1000 por 5000 en las tres apariciones en esta sección y se pidió imprimir el No de iteración a la cual convergió  
  else {
    require(mixtools)
    tmp <- unlist(lapply(regul, function(x) x$tfmode), use.names = F)
    fit <- list(mu = c(-0.5, 0, 0.5), sigma = c(0.15, 0.25, 0.15), lambda = c(0.2, 0.4, 0.4), all.loglik = rep(0, 5000+1))
    while (length(fit$all.loglik) > 5000) fit <- normalmixEM(tmp, mu = fit$mu, sigma = fit$sigma, lambda = fit$lambda, mean.constr = c(NA, 0, NA), maxit = 5000, verb = F)
    print(length(fit$all.loglik))
  }
  #---------------------------------------------------------------------------------------------------
  # Para que funcione correctamente hay que definir manualmente esta función que está incluida en la sección "internal.r" de ssmarina
