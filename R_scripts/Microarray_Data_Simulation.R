...
library(limma)
library(gplots)
load("~/notron/MARINa/Rdatas/general_MARINa.RData")
...
fparams <- data.frame(m1 = 819, m2 = 61, shape2 = 4, lb = 4, ub = 14, pde = 0.02, sym = 0.5)
dparams <- data.frame(lambda1 = 0.13, lambda2 = 2, muminde = 1, sdde = 0.5)

# Generate simulated data
simdset <- madsim(mdata=dset, n= 14652, ratio = 0, fparams, dparams, sdn = 0.4, rseed = 50)

str(simdset)
dim(simdset$xdata)
sim_dset <- simdset$xdata
row.names(sim_dset) <- row.names(dset)
colnames(sim_dset) <- colnames(dset)
...
col.enf <-c(rep("FF0000",819))
col.san <-c(rep("0000FF",61))
patientcolors <- c(rep(0,880))
patientcolors[enfMuest] = col.enf
patientcolors[sanMuest] = col.san

####################                       Real data                       ####################
# estimate log fold change
fit = lmFit(dset, design)
fit2 = contrasts.fit(fit, cont.matrix)
fit2 = eBayes(fit2)

# topTable(fit2, coef=i, adjust='fdr')

##  Heatmap y Volcanoplot
...
TT <- topTable(fit2, coef = i, adjust = 'fdr', n = length(rownames(dset)), genelist = rownames(dset))
selected = TT[(TT$B>5 & abs(TT$logFC)>M) & TT$adj.P.Val<P,]
x <- dset[rownames(selected),]
par(oma = c(3,1,3,4))
...
pdf(file = "Heatmap_volcanoplot.pdf")
volcanoplot(fit2, coef=1, main = contrastes[1], )
heatmap.2(x, trace="none", col=redgreen(75), scale="row", labCol=colnames(dset), labRow=rownames(dset), margins = c(8, 8), cexCol = 0.6,cexRow = 0.6)

volcanoplot(fit2, col="blue", ylim=c(y0[i],y1[i]), xlim=c(x0[i],x1[i]), coef=i, main=contrastes[i], cex.lab=1.3)
    par(new=T)
    ...
    plot(x, y, col="magenta",ylim=c(y0[i],y1[i]), xlim=c(x0[i],x1[i]),main="", pch = "*",xlab="", ylab="",cex.lab=1.2)
    ...
    par(new=T)
    plot(x, y, col="green3",  ylim=c(y0[i],y1[i]), xlim=c(x0[i],x1[i]),main="", pch = 19,xlab="", ylab="",cex.lab=1.2)
#heatmap.2(x, col=redgreen(75), scale="row", key = T, symkey = F, density.info = 'none', cexRow = 1.1, cexCol = 1.5, main = contrastes[i])
dev.off()
...
# estimate log fold change
simfit = lmFit(sim_dset, design)
simfit2 = contrasts.fit(simfit, cont.matrix)
simfit2 = eBayes(simfit2)

# topTable(simfit2, coef=i, adjust='fdr')

##  Heatmap


simTT <- topTable(simfit2, coef = i, adjust = 'fdr', n = length(rownames(dset)), genelist = rownames(dset))
sim_selected = simTT[(simTT$B>B & abs(simTT$logFC)>M) & TT$adj.P.Val<P,]
sim_x <- simdset$xdata[rownames(sim_selected),]

range(simTT$B)
range(simTT$logFC)
range(simTT$adj.P.Val)

pdf(file = "Heatmap_volcanoplot.pdf")
volcanoplot(sim_fit2, coef=1, main = contrastes[1])
heatmap.2(sim_x, col=redgreen(75), scale="row", key = T, symkey = F, density.info = 'none', cexRow = 1.1, cexCol = 1.5, main = contrastes[i])
dev.off()


X=read.table("bySignificanceLevel_B", header=TRUE, sep =",")

# La primera columna del objeto X son los nombres de las comunidades

Names=X[,1]

# Las columnas 2 a la última (en este caso 31) son los valores de los p-values

Y=X[,2:31]

# Tratamos a Y como matriz y no solo como arreglo:

Y=as.matrix(Y)

# Calculamos el valor de intensidad de coor como -log(p-value)
...
my_palette <- colorRampPalette(c("yellow", "white", "red"))(n = 100)

# Cargamos la librería (gplots)

library(gplots)

# Para escribir un PDF (y que sea landscape):

pdf("Cell_cycle_heatmap.pdf", paper="USr")

# Y con esa librería hacemos el heatmap

heatmap.2(Z, trace="none", col=my_palette, scale="row", labCol=colnames(Y), labRow=Names, margins = c(5, 13),cexCol = 0.6,cexRow = 0.6)

# Se cierra el buffer para guardar la imagen

dev.off()


##########################################################################
### Estos parámetros sirven para cargar de nuevo desde la matriz de epresión los datos ya inluidos en general:_MARINa.RData
# dset <- read.table("/home/hachepunto/data/Expression_Matrix_genesym_todos_sin_duplicados_colapsed.txt",header=TRUE,row.names=1)
# dset <- as.matrix(dset)
